...
class MTTRPM_MaskDot_Generator(DataGenerator):
    """Yield two inputs per sample:
       - stance: ([CLS] text [SEP] stance_prompt [SEP]) with mask at '-' (second one)
       - sentiment: ([CLS] text [SEP] sent_prompt [SEP]) with mask at '-'
       Provide:
         - token ids & segment ids for both inputs
         - mask positions for both inputs
         - scalar class labels (0..2) for both tasks
    """
