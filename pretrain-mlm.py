...
def corpus():
    ...
    """
    while True:
        f = modelPath+'/datasets/data_ori/Z-stance-ext.json'
        with open(f) as f:
            for l in f:
                l = json.loads(l)
...
class Evaluator(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if opt.topic_augment:
            bert.save_weights_as_checkpoint(modelPath+'/word-embedding/pretrain/wobert-mlm-Z-stance/bert_model_target.ckpt')  # 保存模型
        else:
            bert.save_weights_as_checkpoint(
                modelPath + '/word-embedding/pretrain/wobert-mlm-Z-stance/bert_model.ckpt')  # 保存模型
...
if __name__ == '__main__':
    ...
    if opt.topic_augment:
        bert.save_weights_as_checkpoint(
            modelPath + '/word-embedding/pretrain/wobert-mlm-Z-stance/bert_model_target.ckpt')  # 保存模型
    else:
        bert.save_weights_as_checkpoint(
            modelPath + '/word-embedding/pretrain/wobert-mlm-Z-stance/bert_model.ckpt')  # 保存模型