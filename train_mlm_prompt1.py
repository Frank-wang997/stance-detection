...
vid_against = tokenizer.token_to_id(u'反对')
vid_none    = tokenizer.token_to_id(u'中立')
vid_favor   = tokenizer.token_to_id(u'支持')
if 0 in (vid_against, vid_none, vid_favor):
    raise ValueError('Vocab must contain stance verbalizers exactly: 反对 / 中立 / 支持')

vid_neg = tokenizer.token_to_id(u'消极')
vid_neu = tokenizer.token_to_id(u'中性')
vid_pos = tokenizer.token_to_id(u'积极')
if 0 in (vid_neg, vid_neu, vid_pos):
    raise ValueError('Vocab must contain sentiment verbalizers exactly: 消极 / 中性 / 积极')

dash_id = tokenizer.token_to_id('-')

# Prompt templates
stance_tpl = u'所以 我 对于 - 的 态度 是 - 。'
sent_tpl   = u'所以 我 的 心情 是 - 。'

class MTTRPM_Generator(DataGenerator):
    ...
    def __iter__(self, random=False):
        ...
        for is_end, (text, target, stance, sentiment) in self.sample(random):
            # stance
            stance_prompt = stance_tpl.replace('-', target, 1)  # replace first '-' with target
            ...
            src_ids = t_ids[:]
            src_ids[m_idx] = tokenizer._token_mask_id  # replace with [MASK]
...
train_gen = MTTRPM_Generator(train_data, batch_size)
valid_gen = MTTRPM_Generator(valid_data, batch_size)
...
def gather_mask_prob(args):
    ...
    pos = K.reshape(pos, (-1,))
    B = K.shape(probs)[0]
    idx = K.stack([K.arange(0, B), pos], axis=1)
    gathered = tf.gather_nd(probs, idx)
    return gathered

s_vec = Lambda(gather_mask_prob, name='gather_stance')([s_probs, s_pos_in])  # (B,V)
e_vec = Lambda(gather_mask_prob, name='gather_sent')([e_probs, e_pos_in])    # (B,V)

stance_vids = K.constant([vid_against, vid_none, vid_favor], dtype='int32')
sent_vids   = K.constant([vid_neg, vid_neu, vid_pos], dtype='int32')

def gather_cols(args):
    x, cols = args  # x: (B,V), cols: (C,)
    return tf.gather(x, cols, axis=1)  # (B,C)

stance_probs = Lambda(gather_cols, name='stance_proj')([s_vec, stance_vids])
sent_probs   = Lambda(gather_cols, name='sent_proj')([e_vec, sent_vids])

def sparse_ce_probs(y_true, y_pred):
    y_true = K.reshape(K.cast(y_true, 'int32'), (-1,))
    return K.mean(K.sparse_categorical_crossentropy(y_true, y_pred, from_logits=False))

stance_loss = Lambda(lambda x: sparse_ce_probs(x[0], x[1]), name='stance_loss')([y_s, stance_probs])
sent_loss   = Lambda(lambda x: sparse_ce_probs(x[0], x[1]), name='sent_loss')([y_e, sent_probs])
...
train_model.add_metric(keras.metrics.sparse_top_k_categorical_accuracy(K.reshape(y_s, (-1,)), stance_probs, k=1),
                       name='stance_acc', aggregation='mean')
train_model.add_metric(keras.metrics.sparse_top_k_categorical_accuracy(K.reshape(y_e, (-1,)), sent_probs, k=1),
...
infer_model = Model([s_token_ids, s_segment_ids, e_token_ids, e_segment_ids, s_pos_in, e_pos_in],
                    [stance_probs, sent_probs])

def evaluate(gen):
    stance_trues, stance_preds = [], []
    for (s_ids, s_segs, e_ids, e_segs, s_pos, e_pos, y_s_, y_e_), _ in gen:
        s_p, _ = infer_model.predict([s_ids, s_segs, e_ids, e_segs, s_pos, e_pos])
        pred = s_p.argmax(axis=-1).tolist()
...
class Evaluator(keras.callbacks.Callback):
    def __init__(self):
        self.best = 0.
        self.path = os.path.join(modelPath, 'output/keras-prompt/mttrpm_mask_fixedverbal_best.weights')
...
if __name__ == '__main__':
    ...
    mlm_encoder.load_weights(os.path.join(modelPath, 'output/keras-prompt/mttrpm_mask_fixedverbal_best.weights'))
